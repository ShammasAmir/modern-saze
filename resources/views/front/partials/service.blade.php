<section class="service py-5">
    <div class="container-lg py-5">
      <div class="service-title">
        <h6 class="fs-6 fw-bold text-center text-primary">{{$servicesection->topic}}</h6>
        <h1 class="text-center fw-bold mt-4">{{$servicesection->subtopic}}</h1>
      </div>
      <div class="row py-5">
        <div class="col-md-6">
          @foreach($services as $service)
          <div class="service-item d-flex px-4 py-5 shadow-sm @if($loop->first) bg-dark @elseif($loop->index == 1) bg-primary mt-4 @elseif($loop->last) bg-dark mt-4 @endif">
              <div class="service-item-img">
                <img src="{{asset('/front/images/service/'.$service->icon)}}" height="80" width="80" alt="">
              </div>
              <div class="service-item-text pe-4">
                <h2 class="text-white fs-3 mb-3">{{$service->title}}</h2>
                <p class="text-white fs-6 mb-3">{{$service->description}}</p>
                <a href="{{$service->link}}" class="btn btn-light text-primary fs-6 fw-bold">بیشتر بخوانید</a>
              </div>
            </div>
          @endforeach
        ...
        <div class="col-md-6 d-flex mt-4 mt-md-0">
          <div class="service-img w-100" style="background-image: url('{{asset('/front/images/'.$servicesection->image)}}');"></div>
        </div>
      </div>
    </div>
  </section>
